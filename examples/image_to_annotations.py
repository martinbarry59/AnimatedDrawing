...
def image_to_annotations(img_fn: str, out_dir: str) -> None:
    ...
    print("accessing drawn_humanoid_detector")
    ...
    skeleton.append({'loc' : [round(x) for x in  kpts[0]             ], 'name': 'nose'          , 'parent': None})
    skeleton.append({'loc' : [round(x) for x in  kpts[1]             ], 'name': 'right_eye'     , 'parent': 'nose'})
    skeleton.append({'loc' : [round(x) for x in  kpts[2]             ], 'name': 'left_eye'      , 'parent': 'nose'})
    skeleton.append({'loc' : [round(x) for x in  kpts[3]             ], 'name': 'right_ear'      , 'parent': 'right_eye'})
    skeleton.append({'loc' : [round(x) for x in  kpts[4]             ], 'name': 'left_ear'      , 'parent': 'left_eye'})

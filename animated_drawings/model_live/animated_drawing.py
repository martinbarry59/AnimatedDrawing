...
from animated_drawings.model_live.time_manager import TimeManager
from animated_drawings.model_live.retargeter import Retargeter
from animated_drawings.model_live.arap import ARAP
from animated_drawings.model_live.joint import Joint
from animated_drawings.model_live.quaternions import Quaternions
from animated_drawings.model_live.vectors import Vectors
...
class AnimatedDrawing(Transform, TimeManager):
    ...
    def update(self):
        ...
        # update the rig's root position and reorient all of its joints
            ...
            self.rig.root_joint.set_position(root)

            self.rig.set_global_orientations(self.live_angles)

        # using new joint positions, calculate new mesh vertex xy positions
        control_points: npt.NDArray[np.float32] = self.rig.get_joints_2D_positions() - root[:2]
        self.vertices[:, :2] = self.arap.solve(control_points) + root[:2]
    ...
    def _set_draw_indices(self, joint_depths: Dict[str, float]):
        # Render torso and head first, arms and hands later
        
        
        ## get all joints 
        depths = {}
        for joint_name in self.joint_to_tri_v_idx.keys():
            if "foot" in joint_name or "knee" in joint_name:
                depths[joint_name] = 0.3
            elif "hand" in joint_name or "elbow" in joint_name:
                depths[joint_name] = 0.5
            else:
                depths[joint_name] = 0
        ## order dic per value
        render_order =dict(sorted(depths.items(), key=lambda item: item[1])).keys()
        
        indices: List[npt.NDArray[np.int32]] = []

        for joint_name in render_order:
